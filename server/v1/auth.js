...
const AuthRouter = () => {
  ...
  router.route("/register").post(async function (req, res, next) {
    try {
      ...
      if (!body.role) {
        ...
        const scopeArray = Array.isArray(requestedScope)
          ? requestedScope
          : [requestedScope];

        // Verificar se todos os scopes são válidos
        const isValidScope = scopeArray.every((scope) =>
    ...
    .put(VerifyToken, async function (req, res, next) {
      try {
        ...
        if (userId !== req.userId) {
          return res
            .status(403)
            .send({ error: "Not authorized to update this user's profile" });
        }
  ...
  router.route("/users").get(VerifyToken, async function (req, res, next) {
    try {
      ...
      // Chamar o serviço para buscar os usuários com paginação e filtros
      const users = await userService.findAll(page, limit, query);
